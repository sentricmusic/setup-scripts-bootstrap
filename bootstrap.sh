...
main() {
    ...
    echo ""
    echo -e "  ${WHITE}Run this command to complete setup:${NC}"
    echo ""
    echo -e "    ${YELLOW}cd \"$workspacePath\" && ./setup.sh${NC}"
    echo ""
    echo -e "  ${GRAY}Or with options:${NC}"
    echo -e "    ${GRAY}./setup.sh --interactive    # Interactive mode${NC}"
    echo -e "    ${GRAY}./setup.sh --step 4         # Skip to step 4${NC}"
    echo -e "    ${GRAY}./setup.sh --help           # Show all options${NC}"
    echo ""
}
